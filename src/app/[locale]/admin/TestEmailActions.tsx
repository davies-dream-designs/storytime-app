...
export default function TestEmailActions() {
  ...
  async function send() {
    ...
    try {
      ...
      const data = await res.json() as { sentTo?: string; results?: Record<string, string>; error?: string };
  ...
  return (
    <div className="rounded-2xl border border-night-100 bg-white p-6 shadow-sm mb-8">
      <h2 className="font-display text-lg font-bold text-night-800 mb-1">Test emails</h2>
      <p className="text-sm text-night-400 mb-4">Fires both transactional emails (order confirmation + shipped) to your account email.</p>
      <button
        onClick={send}
