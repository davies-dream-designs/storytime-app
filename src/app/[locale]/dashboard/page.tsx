    ...
    <>
      ...
      <main className="mx-auto max-w-6xl px-5 py-10">
        ...
        <div className="mb-10 grid gap-4 sm:grid-cols-3">
          {[
            { label: t('statProfiles'), value: profiles.length, icon: '👶', href: '/profiles' },
            { label: t('statStories'), value: stories.length, icon: '📖', href: '/stories' },
            {
              label: t('statLastStory'),
              value: recentStories[0] ? new Date(recentStories[0].createdAt).toLocaleDateString(undefined, { day: 'numeric', month: 'short' }) : '—',
              icon: '✨',
        ...
        <div className="mb-10 grid gap-4 sm:grid-cols-2">
          <Link href="/stories/new" className="flex items-center gap-4 rounded-2xl bg-night-700 px-6 py-5 text-white transition hover:bg-night-600">
            ...
            <div>
              <p className="font-display text-lg font-bold">{t('generateTitle')}</p>
          ...
          <Link href="/profiles/new" className="flex items-center gap-4 rounded-2xl border-2 border-dashed border-night-200 px-6 py-5 text-night-600 transition hover:border-night-400 hover:text-night-800">
            <span className="text-3xl" aria-hidden>👶</span>
            <div>
              <p className="font-display text-lg font-bold">{t('addProfileTitle')}</p>
        ...
        {recentStories.length > 0 && (
          <section>
            <div className="mb-5 flex items-center justify-between">
              <h2 className="font-display text-2xl font-bold text-night-800">{t('recentStories')}</h2>
              <Link href="/stories" className="text-sm font-bold text-star-500 hover:text-star-600">{t('viewAll')}</Link>
            </div>
              ...
              {recentStories.map((story) => (
                <Link key={story.id} href={`/stories/${story.id}`} className="group rounded-2xl border border-night-100 bg-white p-5 shadow-sm transition hover:shadow-md">
                  ...
                  <p className="mt-1 text-sm text-night-400">
                    {t('storyFor', { name: story.profileName })} · {new Date(story.createdAt).toLocaleDateString(undefined, { day: 'numeric', month: 'short' })}
                  </p>
                  <p className="mt-1 text-xs text-night-300">{t('wordCount', { count: story.wordCount })} · {t('pageCount', { count: story.pages.length })}</p>
                </Link>
              ))}
            </div>
          </section>
