    ...
    <>
      ...
      <main className="mx-auto max-w-6xl px-5 py-10">
        <div className="mb-8 flex items-center justify-between">
          ...
          <Link href="/profiles/new" className="rounded-full bg-night-700 px-5 py-2.5 text-sm font-bold text-moon-200 transition hover:bg-night-600">
            {t('addChild')}
          </Link>
        </div>
        ...
        {profiles.length === 0 ? (
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {await Promise.all(profiles.map(async (profile) => {
              ...
              return (
                <div key={profile.id} className="relative">
                  <Link href={`/profiles/${profile.id}` as string} className="group block rounded-2xl border border-night-100 bg-white p-6 pb-16 shadow-sm transition hover:shadow-md">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="rounded-full bg-night-50 px-3 py-1 text-sm font-bold text-night-500">{t('ageLabel', { age: profile.age })}</span>
                    </div>
                    ...
                    {profile.favouriteCharacters.length > 0 && (
                      <p className="mt-1 text-sm text-night-400 line-clamp-1">{t('lovesLabel', { items: profile.favouriteCharacters.join(', ') })}</p>
                    )}
                    <p className="mt-4 text-sm text-night-400">{storyCount === 1 ? t('storyCount', { count: storyCount }) : t('storiesCount', { count: storyCount })}</p>
                  </Link>
                  <Link href={`/stories/new?profileId=${profile.id}` as string} className="absolute bottom-5 right-5 z-10 rounded-full bg-night-700 px-4 py-1.5 text-xs font-bold text-moon-200 transition hover:bg-night-600">
                    {t('generate')}
                  </Link>
                </div>
              )
            ...
            <Link href="/profiles/new" className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-night-200 p-6 text-night-400 transition hover:border-night-400 hover:text-night-600">
              ...
              <span className="mt-2 font-display font-bold">{t('addAnother')}</span>
            </Link>
          </div>
        )}
      </main>
