    ...
    <>
      ...
      <div className="storycot-page flex flex-col justify-center bg-parchment px-16 py-16">
        <div className="mb-12 border-b border-night-200 pb-6">
          ...
          <h2 className="mt-3 font-display text-4xl font-bold text-night-800">
            {story.title}
          </h2>
        ...
        {story.pages.slice(0, 2).map((p, i) => (
          <p
            key={i}
            className="mb-6 font-display text-xl leading-relaxed text-night-800"
          >
            {p.text}
          </p>
        ))}
      ...
      {Array.from(
        { length: Math.ceil((story.pages.length - 2) / 2) },
        (_, sheetIdx) => {
          ...
          return (
            <div
              key={sheetIdx}
              className="storycot-page flex flex-col justify-center bg-parchment px-16 py-16"
            >
              {pagePair.map((p, i) => (
                <p
                  key={i}
              ...
              <p className="mt-auto text-right text-xs text-night-300">
                {sheetIdx + 2}
              </p>
            </div>
          );
      ...
      <div className="storycot-page flex flex-col items-center justify-center bg-night-800 p-12 text-center text-white">
        ...
        <p className="mt-8 font-display text-3xl font-bold text-moon-200">
          {t("theEnd")}
        </p>
        <p className="mt-4 text-lg text-white/70">
          {t("sweetDreams", { name: story.profileName })}
        </p>
