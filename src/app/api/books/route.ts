...
export async function GET() {
  ...
  const sorted = [...projects].sort((a, b) => (a.createdAt > b.createdAt ? -1 : 1))
...
export async function POST(req: NextRequest) {
  ...
  if (!sourceStoryId) {
    return NextResponse.json({ error: 'sourceStoryId is required' }, { status: 400 })
  }
