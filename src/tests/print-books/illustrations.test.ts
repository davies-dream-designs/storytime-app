    ...
    pages: [
      {
        ...
        illustrationPrompt: 'A magical hardcover picture-book cover moment in a moonlight garden.',
      },
    ],
  }
}
...
describe('generateCoverIllustration', () => {
  ...
  it('builds a cover prompt from the character bible and cover spread', async () => {
    ...
    expect(prompt).toContain('Child appearance: Mila has curly dark hair and bright brown eyes.')
  ...
  it('creates a placeholder spread asset when provider credentials are missing', async () => {
    const { generateSpreadIllustration } = await import('@/lib/print-books/illustrations')
    const project = createProject()
    const spread = {
      ...project.spreads[0]!,
      id: 'book-1:spread:2',
      ...
      pageEnd: 4,
      title: 'Title',
      illustrationPrompt: 'A gentle title-page illustration motif for "Moonlight Garden".',
    }
